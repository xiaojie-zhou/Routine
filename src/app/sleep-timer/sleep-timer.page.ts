...
export class SleepTimerPage implements OnInit {
  ...
  terminateTimer() {
    ...
    if (this.interValId) {
      ...
      this.showSummary();
      this.logOvernight();
      this.sleepService.onlylogOvernightData(new OvernightSleepData(this.from, this.to));
      this.sleepService.currH = this.from.getHours();
      this.sleepService.currM = this.from.getMinutes();
    }
  }
